...
import { getPaymentInfoServiceType } from "components/sales/types";
...
const Transfer = (props: TransferProps) => {
  ...
  const paymentInfoQuery = useQuery(
    ...
    () => props.getPaymentInfoService({ goodCode: props.goodCode }),
    {
  ...
  const orderInfo = useMemo(() => {
    const _orderInfo = paymentInfoQuery?.data?.content?.orderInfo;

    return _orderInfo?.paymentChannel === "VACC_VPBANK"
      ? _orderInfo
      : undefined;
  }, [paymentInfoQuery?.data?.content?.orderInfo]);

  const expried = useMemo(() => {
    const _expiredTime = orderInfo?.expiredTime;
  ...
  const fee = useMemo(() => {
    ...
    if (orderInfo?.details?.length > 0) {
      orderInfo?.details?.forEach((it: any) => {
        _chargeAmount += it?.chargeAmount;
        _discountAmount += it?.discountAmount;
      });
    }

    return _chargeAmount - _discountAmount;
  }, [orderInfo?.details]);
    ...
    >
      <div className={classes.transferWrapper}>
        ...
        <Typography component="div" className={classes.account}>
          Số tài khoản*: <b>{orderInfo?.virtualAccount}</b>
          <IconButton
            sx={{ marginLeft: "4px" }}
            size="small"
            onClick={() => {
              navigator.clipboard.writeText(orderInfo?.virtualAccount);
...
export default withTheme<TransferProps>(Transfer);
