...
const SaleGolferSeaBank = (props: SaleGolferSeaBankProps) => {
        ...
        onNext={async (
          ...
          if (_currentStepIndex === 1) {
            ...
            if (_res?.message?.indexOf("UNIQUE_TPL_DATA") >= 0) {
              ...
              useFormResult?.setValue(
                fields.totalFee,
                _res?.content?.response?.polTotalFee
              );
              if (!!props.onNextStep) {
                props.onNextStep(_currentStepIndex, _currentScreenIndexOfStep);
              }
...
export default withTheme<SaleGolferSeaBankProps>(SaleGolferSeaBank);
