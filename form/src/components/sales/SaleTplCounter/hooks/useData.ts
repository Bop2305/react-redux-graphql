...
const useData = (props: useDataProps) => {
  ...
  const bookVoucherPolicyMutation = useMutation(props.bookVoucherPolicyService);
  ...
  const getProgramsQuery = useQuery(
    ["getPrograms", props.typeFlow],
    props.getProgramsService,
    {
      refetchOnWindowFocus: false,
    ...
    async (contextMode, e) => {
      ...
      const _data = {
        ...
        vehicleSeatCount: Number(
          useFormResult.getValues(fields.vehicleSeatCount) || 0
        ),
  ...
  const paymentHandle = useCallback(async () => {
    return await paymentMutation.mutateAsync({
      ...
      successUrl: `${
        ...
        props.successEndpoint && props.successEndpoint.indexOf("?") >= 0
          ? `&id=${createPolicyMutation?.data?.content?.response?.polSumId}`
          : `?id=${createPolicyMutation?.data?.content?.response?.polSumId}`
      }`,
