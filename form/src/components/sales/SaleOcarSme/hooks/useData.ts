...
const useData = (props: useDataProps) => {
  ...
  const loanContractTypeData = useMemo(
    () =>
      getPlaceOrderInitQuery?.data?.content?.orderSpecs?.loanContractType
        ?.items,
    [getPlaceOrderInitQuery?.data?.content?.orderSpecs?.loanContractType?.items]
  );
  ...
  const createPolicyHandle = useCallback(() => {
    createPolicyMutation.mutate({
      ...
      orderId: pendingOrderData?.orderId,
      debitAccount: useFormResult.getValues(fields.debitAccount),
  ...
  return {
    ...
    // staff info
    getStaffContractByCodeData: getStaffContractByCodeQuery?.data,
    getStaffContractByCodeLoading:
      getStaffContractByCodeQuery?.isInitialLoading,
    // getPremiumEstQuery
    getPremiumEstQuery,
    hierarchies,
    getStaffInfoQuery,
