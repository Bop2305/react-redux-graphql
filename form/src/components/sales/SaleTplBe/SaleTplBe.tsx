...
const SaleTplBe = (props: SaleTplBeProps) => {
  ...
  const {
    ...
    bookVoucherPolicyService: props.bookVoucherPolicyService,
    // props
  ...
  const steps = useSteps({
    ...
    onChangeTypeFlow: props.onChangeTypeFlow,
  });
    ...
    <div className={`${classes.wrapper} ${props.className}`}>
      <Stepper
        ...
        onBack={(_data, _currentStepIndex, _currentScreenIndexOfStep, back) => {
          if (!!props.onBackStep) {
            props.onBackStep(_currentStepIndex, _currentScreenIndexOfStep);
          }
        }}
        onNext={async (
          _data,
          ...
          if (_currentStepIndex === 1) {
            ...
            if (_res?.message?.indexOf("UNIQUE_TPL_DATA") >= 0) {
              ...
              bookVoucherPolicyMutation.mutate({
...
export default withTheme<SaleTplBeProps>(SaleTplBe);
