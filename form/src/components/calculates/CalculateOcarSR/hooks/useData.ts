...
const useData = (props: useDataProps) => {
  ...
  const cityDataDefault = useMemo(
    () => new URLSearchParams(window.location.search)?.get('locationCode'),
    [getProvincesQuery?.data?.content]
  );
  ...
  const carBrandDefaultData = useMemo(
    () => new URLSearchParams(window.location.search)?.get('carBrand'),
    [getCarBrandsQuery?.data?.content]
  );
